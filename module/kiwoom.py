...
class api():
    ...
    def comm_rq_data(self, sRQName, sTrCode, nPrevNext, sScreenNo):
        ...
        OP_ERR_NONE(0) – 정상처리
        """
        return self.ocx.dynamicCall("CommRqData(QString, QString, QString, QString)", sRQName, sTrCode, nPrevNext,
                                    sScreenNo)
    ...
    def OnReceiveTrData(self, sScrNo, sRQName, sTrCode, sRecordName, sPreNext, candle=None):
        ...
        if sRQName == '미결제잔고내역조회':
            ...
            order_contents = {}
            contract_cnt = int(
                self.ocx.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode, sRecordName, 1, '매도수량'))
            ...
            try:
                ...
                if subject.info[subject_code]['전략'] == "풀파라" or subject.info[subject_code]['전략'] == "리버스온리":
                    if (calc.data[subject_code]['플로우'][-1] == "상향" and order_contents['매도수구분'] == '신규매도') or (
            ...
            for i in range(0, 20):
                subject_symbol = self.ocx.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode,
                                                      sRecordName, i, '파생품목코드').strip()
            ...
            for subject_code in subject.info.keys():
                ...
                if sScrNo == subject.info[subject_code]['화면번호']:
                    if subject_code not in calc.data or calc.data[subject_code]['idx'] == -1:
                        ...
                        if d.get_mode() == d.REAL:
                            datas = self.ocx.dynamicCall("GetCommFullData(QString, QString, int)", sTrCode, sRecordName,
                                                         0)
                            datas = datas.split()
                ...
                if sScrNo == subject.info[subject_code]['화면번호']:
                    ...
                    if subject_code not in calc.data or calc.data[subject_code]['idx'] == -1:
                        ...
                        if d.get_mode() == d.REAL:
                            ...
                            if subject_code in self.candle_data.keys() and len(
                                    self.candle_data[subject_code]) > 9000 * 7:

                                # 수신된 전체데이터를 반환한다.(feat.키움open_api pdf)
                                data = self.ocx.dynamicCall("GetCommFullData(QString, QString, int)", sTrCode,
                                                            sRecordName, 0)
                                data = data.split()
                                ...
                                # 최근가
                                self.recent_price[subject_code] = round(float(self.candle_data[subject_code][1]),
                        ...
                        # 안씀
                        for idx in range(0, 10): self.recent_price_list[subject_code].append(
                            self.recent_price[subject_code])  # 현재가 삽입
                        # 안씀
                        self.adjusted_price[subject_code] = round(
                            float(sum(self.recent_price_list[subject_code])) / max(
                                len(self.recent_price_list[subject_code]), 1), subject.info[subject_code]['자릿수'])
                        if d.get_mode() == d.REAL: return

                    # 틱INFO라서 처음에 600캔들 요청하고 나면 data가 만들어져 있으니 이후에 price에 추가
                    if d.get_mode() == d.REAL:  # 실제투자
                        price['현재가'] = self.ocx.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode,
                                                            sRecordName, 1, '현재가')
                        price['저가'] = self.ocx.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode,
                                                           sRecordName, 1, '저가')
                        price['고가'] = self.ocx.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode,
                                                           sRecordName, 1, '고가')
                        price['시가'] = self.ocx.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode,
                                                           sRecordName, 1, '시가')
                        price['체결시간'] = self.ocx.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode,
                                                             sRecordName, 1, '체결시간')
                        price['거래량'] = self.ocx.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode,
                                                            sRecordName, 1, '거래량')
                    ...
                    # 캔들이 갱신되었는지 확인
                    if self.recent_candle_time[subject_code] != price['체결시간'] or subject_code not in self.last_price or \
        ...
        if sRQName == '상품별현재가조회':
            ...
            for i in range(20):
                ...
                subject_code = self.ocx.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode, sRecordName,
                                                    i, '종목코드n').strip()  # 현재가 = 틱의 종가
        ...
        if sRQName == "장운영정보조회":
            ...
            for i in range(20):
                subject_code = self.ocx.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode, sRecordName,
                                                    i, '파생품목코드')
                market_time1 = self.ocx.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode, sRecordName,
                                                    i, '장운영시간1')
                market_time2 = self.ocx.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode, sRecordName,
                                                    i, '장운영시간2')
                market_time3 = self.ocx.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode, sRecordName,
                                                    i, '장운영시간3')
                market_time4 = self.ocx.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode, sRecordName,
                                                    i, '장운영시간4')
    ...
    def OnReceiveRealData(self, subject_code, sRealType, sRealData):
        ...
        if sRealType == '해외선물시세':
            ...
            self.adjusted_price[subject_code] = round(
                float(sum(self.recent_price_list[subject_code])) / max(len(self.recent_price_list[subject_code]), 1),
                subject.info[subject_code]['자릿수'])
                    ...
                    if sell_contents['신규주문'] == True:
                        ...
                        if d.get_mode() == d.REAL:  # 실제투자
                            if order_result != 0:
                                ...
                                if next_state == '청산시도중':
                                    log.info("%s 종목 %s %s개 청산요청." % (
                                    subject_code, sell_contents['매도수구분'], sell_contents['수량']))
                                elif next_state == '매매시도중':
                                    log.info(
                                        "%s 종목 %s %s개 매매요청." % (
                                        subject_code, sell_contents['매도수구분'], sell_contents['수량']))
                        '''
                    ...
                    if order_contents['신규주문'] == True:
                        ...
                        if d.get_mode() == d.REAL:  # 실제투자
                            if order_result != 0:
                                ...
                                log.info(
                                    "%s 종목 %s %s개 요청." % (subject_code, order_contents['매도수구분'], order_contents['수량']))
                                try:
                ...
                if subject_code in contract.list:
                    if contract.get_contract_count(subject_code) > 0:
                        ...
                        if order_info['매도수구분'] == 1:
                            profit = ((float(order_info['체결표시가격']) - float(contract.list[subject_code]['체결가'])) /
                                      subject.info[subject_code]['단위'] * subject.info[subject_code][
                                          '틱가치'] - contract.get_commission(self.account)) * remove_cnt
                        elif order_info['매도수구분'] == 2:
                            profit = ((float(contract.list[subject_code]['체결가'] - float(order_info['체결표시가격']))) /
                                      subject.info[subject_code]['단위'] * subject.info[subject_code][
                                          '틱가치'] - contract.get_commission(self.account)) * remove_cnt

                        if order_info['매도수구분'] == 1:
                            full_para.previous_profit = (
                            (float(order_info['체결표시가격']) - float(contract.list[subject_code]['체결가'])) /
                            subject.info[subject_code]['단위'])
                        elif order_info['매도수구분'] == 2:
                            full_para.previous_profit = (
                            (float(contract.list[subject_code]['체결가'] - float(order_info['체결표시가격']))) /
                            subject.info[subject_code]['단위'])
